      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left">
          <thead className="bg-surface-container-low/50">
            <tr>
              <th className="px-10 py-4 text-[10px] font-extrabold uppercase tracking-widest text-on-surface-variant/70">Course Code</th>
              <th className="px-6 py-4 text-[10px] font-extrabold uppercase tracking-widest text-on-surface-variant/70">Description</th>
              <th className="px-6 py-4 text-[10px] font-extrabold uppercase tracking-widest text-on-surface-variant/70 text-right">Grade</th>
              <th className="px-10 py-4 text-[10px] font-extrabold uppercase tracking-widest text-on-surface-variant/70 text-right">Status</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-outline-variant/10">
            {!hasCourses ? (
              <tr>
                <td colSpan={4} className="px-10 py-12 text-center text-sm text-on-surface-variant italic">
              ...
              sortedCourses.map((course) => (
                ...
                >
                  <td className="px-10 py-6 text-sm font-bold text-on-surface">
                    <div className="flex items-center gap-3">
                      <div className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: course.course_color || "#3f6900" }} />
                      {course.course_code || "N/A"}
                    </div>
                  </td>
                  <td className="px-6 py-6 text-sm text-on-surface-variant font-medium">{course.course_name}</td>
                  <td className="px-6 py-6 text-right font-bold text-on-surface">{course.grade ? course.grade.toFixed(2) : "---"}</td>
